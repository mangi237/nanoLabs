            ...
            {currentStep === 1 && (
              <div className="space-y-4">
                ...
                {!showLabSelector && selectedLab ? (
                  ...
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-xs text-teal-300 font-semibold flex items-center gap-1.5 mt-0.5 truncate">
                          <MapPin className="w-3.5 h-3.5 text-teal-400 shrink-0" />
                          <span>
                            {selectedLab.city ? `${selectedLab.city} • ` : ''}
                            {selectedLab.address || selectedLab.location || 'Central Health Diagnostic Hub'}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="bg-slate-900 border border-teal-500/50 rounded-2xl p-4 space-y-3 shadow-xl">
                    <div className="relative">
                      <Search className="w-4 h-4 text-teal-400 absolute left-3.5 top-3" />
                      <input
                        type="text"
                        placeholder="Search lab center by name or city (e.g. Douala, Yaoundé, Bafoussam)..."
                        value={labSearchQuery}
                        onChange={e => setLabSearchQuery(e.target.value)}
                        className="w-full pl-10 pr-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-400 text-xs focus:outline-none focus:ring-2 focus:ring-teal-400"
                        autoFocus
                      />
                    </div>
                          ...
                          >
                            <div className="flex items-center gap-3 min-w-0">
                              ...
                              <div className="min-w-0 text-xs">
                                <div className="font-bold truncate text-sm text-white">{l.name}</div>
                                <div className="text-[11px] text-teal-300 truncate">
                                  {l.city ? `${l.city} • ` : ''}
                                  {l.address || l.location || 'Diagnostic Center'}
                                </div>
                              </div>
                            </div>
                            {selectedLab?.id === l.id && (
