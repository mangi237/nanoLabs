...
export const ReportsScreen: React.FC<ReportsScreenProps> = ({
  ...
  const [financialInsuranceFilter, setFinancialInsuranceFilter] = useState('All');
  ...
  const fetchData = async () => {
    try {
      ...
      // 1b. Fetch Bookings (Walk-in & cashier orders)
      let bookingList: any[] = [];
      try {
        const bookSnap = await getDocs(collection(db, 'labs', targetLabId, 'bookings'));
        bookingList = bookSnap.docs.map(d => ({ id: d.id, ...d.data() }));
        setBookings(bookingList);
      } catch (bErr) {
        console.warn('Notice loading bookings for reports:', bErr);
      }
  ...
  const testsFromPatients = patients.flatMap(p => {
    const pName = p.name || p.fullName || 'Anonymous Patient';
    const pMatricule = p.insuranceNumber || p.matricule || p.patientCode || p.id?.slice(0, 8) || 'MAT-001';
    const insProvider = p.insuranceProvider || p.insuranceName || 'Cash / Direct';
    const isInsured = Boolean(insProvider && insProvider !== 'None' && insProvider !== 'Cash / Direct' && insProvider !== 'Direct Cash');
    const covRate = p.coverageRate || p.insurancePercentage || (isInsured ? 80 : 0);
    ...
    return (p.labTests || []).map((t: any) => {
      ...
      const testInsRate = t.coverageRate !== undefined ? t.coverageRate : covRate;
      const insShare = t.insuranceShare !== undefined 
        ? t.insuranceShare 
        : (testInsRate > 0 ? Math.round((totPrice * testInsRate) / 100) : 0);
      const patCopay = t.patientCopay !== undefined 
        ? t.patientCopay 
        : (totPrice - insShare);
      ...
      return {
        ...
        patientId: p.id,
        patientName: pName,
        patientMatricule: pMatricule,
        insuranceProvider: t.insuranceProvider || insProvider,
        isInsured: isInsured || Boolean(t.insuranceProvider && t.insuranceProvider !== 'Cash / Direct'),
        coverageRate: testInsRate,
        insuranceShare: insShare,
        patientCopay: patCopay,
        actCode: t.code || t.actCode || 'ACT-LAB',
        cote: t.cote || 'B30',
        ...
        paymentMethod: t.paymentMethod || (isInsured ? 'Insurance + Copay' : 'Cash'),
  ...
  const testsFromBookings = bookings.flatMap(b => {
    const pName = b.patientName || 'Patient';
    const pMatricule = b.patientMatricule || b.matricule || b.insuranceNumber || b.id?.slice(0, 8) || 'MAT-001';
    const insProvider = b.insuranceProvider || b.insuranceName || (b.isInsured ? 'ASCOMA Cameroun' : 'Cash / Direct');
    const isInsured = Boolean(b.isInsured || (insProvider && insProvider !== 'None' && insProvider !== 'Cash / Direct' && insProvider !== 'Direct Cash'));
    const covRate = b.insuranceCoverageRate || b.coverageRate || (isInsured ? 80 : 0);

    return (b.tests || []).map((t: any, tIdx: number) => {
      const sFee = defaultFee;
      const totPrice = t.price || 5000;
      const bPrice = totPrice > sFee ? totPrice - sFee : totPrice;
      const testInsRate = t.coverageRate !== undefined ? t.coverageRate : covRate;
      const insShare = t.insuranceShare !== undefined 
        ? t.insuranceShare 
        : (testInsRate > 0 ? Math.round((totPrice * testInsRate) / 100) : 0);
      ...
      return {
        ...
        insuranceShare: insShare,
        patientCopay: patCopay,
        actCode: t.testCode || t.actCode || t.code || 'ACT-LAB',
        cote: t.cote || 'B30',
        testName: t.testName || t.name || 'Laboratory Test',
        category: t.category || 'General Diagnostics',
        basePrice: bPrice,
        ...
        totalPrice: totPrice,
        price: totPrice,
        paymentStatus: b.paymentStatus || (b.status === 'Completed' ? 'paid' : 'unpaid'),
        paymentMethod: b.paymentMethod || (isInsured ? 'Insurance + Copay' : 'Cash'),
        receiptNumber: b.bookingCode || b.receiptNumber || `FACT-${b.id?.slice(0, 6)}`,
        status: t.status === 'Completed' ? 'completed' : 'processing',
        date: b.date || b.createdAt?.split('T')[0] || '2026-08-06',
        specimenType: t.sampleType || 'Venous Blood',
        verifiedBy: t.completedBy || (t.status === 'Completed' ? 'Dr. Sarah (Lab Tech)' : 'Pending'),
        collectedBy: b.sampleCollectedBy || (b.sampleCollected ? 'Jean (Phlebotomist)' : 'Pending Intake'),
        cashierName: b.paymentProcessedBy || 'Cashier Counter'
      };
    });
  });

  const allTests = [...testsFromPatients, ...testsFromBookings];
  ...
  const content = (
    <div className="space-y-6">
      ...
      {/* ------------------------------------------------------------- */}
      {/* 5. FINANCIAL & CAISSE INSURANCE BILLING REPORTS */}
      {/* ------------------------------------------------------------- */}
      {activeCategory === 'financial' && (() => {
        // Financial filtered tests
        const paidTests = filteredTests.filter(t => t.paymentStatus === 'paid' || t.status === 'completed');
        
        const matchingFinancialTests = paidTests.filter(t => {
          const q = financialSearchQuery.toLowerCase().trim();
          const matchesSearch = !q || 
            (t.patientName || '').toLowerCase().includes(q) ||
            (t.patientMatricule || '').toLowerCase().includes(q) ||
            (t.insuranceProvider || '').toLowerCase().includes(q) ||
            (t.testName || '').toLowerCase().includes(q) ||
            (t.receiptNumber || '').toLowerCase().includes(q) ||
            (t.actCode || '').toLowerCase().includes(q) ||
            (t.cote || '').toLowerCase().includes(q);

          const matchesInsurance = financialInsuranceFilter === 'All' ||
            (financialInsuranceFilter === 'Cash' && (!t.isInsured || t.insuranceProvider === 'Cash / Direct')) ||
            (t.insuranceProvider === financialInsuranceFilter);

          return matchesSearch && matchesInsurance;
        });

        const grossTotal = matchingFinancialTests.reduce((sum, t) => sum + (t.totalPrice || 0), 0);
        const insuranceTotal = matchingFinancialTests.reduce((sum, t) => sum + (t.insuranceShare || 0), 0);
        const patientCopayTotal = matchingFinancialTests.reduce((sum, t) => sum + (t.patientCopay || 0), 0);
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-1">
                <span className="text-xs font-semibold text-slate-500 uppercase">Gross Amount Invoiced</span>
                <div className="text-2xl font-black text-slate-900">{grossTotal.toLocaleString()} FCFA</div>
                <p className="text-[11px] text-slate-500 font-medium">{matchingFinancialTests.length} billing transactions</p>
              </div>
              <div className="bg-white rounded-2xl p-5 border border-indigo-200 shadow-xs space-y-1 bg-indigo-50/30">
                <span className="text-xs font-semibold text-indigo-900 uppercase flex items-center gap-1">
                  <Shield className="w-3.5 h-3.5 text-indigo-600" />
                  Insurance / Caisse Share
                </span>
                <div className="text-2xl font-black text-indigo-700">{insuranceTotal.toLocaleString()} FCFA</div>
                <p className="text-[11px] text-indigo-700 font-medium">Reimbursable from Third-Party Insurers</p>
              </div>
              <div className="bg-white rounded-2xl p-5 border border-emerald-200 shadow-xs space-y-1 bg-emerald-50/30">
                <span className="text-xs font-semibold text-emerald-900 uppercase flex items-center gap-1">
                  <Receipt className="w-3.5 h-3.5 text-emerald-600" />
                  Patient Ticket Modérateur
                </span>
                <div className="text-2xl font-black text-emerald-700">{patientCopayTotal.toLocaleString()} FCFA</div>
                <p className="text-[11px] text-emerald-700 font-medium">Direct Copay Collected at Cashier Counter</p>
              </div>
              <div className="bg-white rounded-2xl p-5 border border-teal-200 shadow-xs space-y-1 bg-teal-50/30">
                <span className="text-xs font-semibold text-teal-900 uppercase flex items-center gap-1">
                  <DollarSign className="w-3.5 h-3.5 text-teal-600" />
                  Direct Lab Net Revenue
                </span>
                <div className="text-2xl font-black text-teal-700">{(grossTotal - totalSystemFees).toLocaleString()} FCFA</div>
                <p className="text-[11px] text-teal-700 font-medium">Excludes nanoLabs platform cloud fees</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-4 border border-slate-200 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
              <div className="relative w-full md:w-96">
                ...
                <input
                  type="text"
                  placeholder="Search by Patient Name, Insurance, Matricule, Receipt #..."
                  value={financialSearchQuery}
                  onChange={e => setFinancialSearchQuery(e.target.value)}
                  className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:bg-white transition-all"
                />
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                <div className="flex items-center gap-1.5 bg-slate-50 px-3 py-1.5 rounded-xl border border-slate-200 text-xs text-slate-600 shrink-0">
                  <Building2 className="w-3.5 h-3.5 text-slate-500" />
                  <span className="font-semibold text-slate-700">Filter Caisse:</span>
                </div>
                <select
                  value={financialInsuranceFilter}
                  onChange={e => setFinancialInsuranceFilter(e.target.value)}
                  className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-600 cursor-pointer"
                >
                  <option value="All">All Providers & Cash Patients</option>
                  <option value="Cash">Direct Cash (No Insurance)</option>
                  <optgroup label="Cameroon Insurance Companies">
              ...
              <div className="p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div>
                  <h3 className="font-bold text-slate-900 text-sm sm:text-base">
                    Insurance Caisse & Cashier Financial Ledger
                  </h3>
                  <p className="text-xs text-slate-500">
                    Comprehensive Cameroon insurance breakdown (Facture Externe) with COTE acts, Taux Assu share, and Patient Copay
                  </p>
                </div>
                <div className="text-xs font-semibold text-slate-500">
                  Showing <span className="font-bold text-slate-900">{matchingFinancialTests.length}</span> invoices
                </div>
              </div>
                ...
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 text-slate-500 font-semibold uppercase tracking-wider border-b border-slate-100">
                    <tr>
                      ...
                      <th className="px-5 py-3.5">Patient Name & Matricule</th>
                      <th className="px-5 py-3.5">Insurance Company / Caisse</th>
                      <th className="px-5 py-3.5">Act & Test Details</th>
                      <th className="px-5 py-3.5 font-mono">Gross Total</th>
                      <th className="px-5 py-3.5 font-mono text-indigo-700">Insurance Share</th>
                      <th className="px-5 py-3.5 font-mono text-emerald-700">Patient Copay</th>
                      <th className="px-5 py-3.5">Payment Method</th>
                      <th className="px-5 py-3.5 text-right">Date</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                    {matchingFinancialTests.length === 0 ? (
                      <tr>
                        <td colSpan={9} className="px-6 py-10 text-center text-slate-400">
                          No financial transactions matching the selected insurance provider or search query.
                        </td>
                      </tr>
                    ) : (
                      matchingFinancialTests.map((t, idx) => (
                        <tr key={idx} className="hover:bg-slate-50/80 transition-colors">
                          <td className="px-5 py-4 font-mono font-bold text-teal-700">
                            {t.receiptNumber !== 'N/A' ? t.receiptNumber : `FACT-${idx + 1001}`}
                          </td>
                          <td className="px-5 py-4">
                            <div className="font-bold text-slate-900">{t.patientName}</div>
                            <div className="text-[10px] text-slate-500 font-mono">Mat: {t.patientMatricule}</div>
                          </td>
                          <td className="px-5 py-4">
                            {t.isInsured ? (
                              <div>
                                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md bg-indigo-50 text-indigo-800 border border-indigo-200 font-bold text-[10px]">
                                  <Shield className="w-3 h-3 text-indigo-600" />
                                  {t.insuranceProvider}
                                </span>
                                <div className="text-[10px] text-slate-500 font-medium mt-0.5">
                                  Taux: {t.coverageRate}% / Copay: {100 - t.coverageRate}%
                                </div>
                              </div>
                            ) : (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-semibold text-[10px]">
                                Direct Cash (100% Patient)
                              </span>
                            )}
                          </td>
                          <td className="px-5 py-4">
                            ...
                            <div className="text-[10px] text-slate-500 font-mono flex items-center gap-1.5 mt-0.5">
                              <span className="bg-slate-100 px-1.5 py-0.2 rounded text-slate-700">{t.actCode}</span>
                              <span>COTE: {t.cote}</span>
                            </div>
                          </td>
                          <td className="px-5 py-4 font-mono font-bold text-slate-900">
                            {(t.totalPrice || 0).toLocaleString()} FCFA
                          </td>
                          <td className="px-5 py-4 font-mono font-bold text-indigo-700 bg-indigo-50/20">
                            {(t.insuranceShare || 0).toLocaleString()} FCFA
                          </td>
                          <td className="px-5 py-4 font-mono font-black text-emerald-700 bg-emerald-50/20">
                            {(t.patientCopay || 0).toLocaleString()} FCFA
                          </td>
                          <td className="px-5 py-4">
