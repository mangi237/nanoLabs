    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-md space-y-6">
          ...
          {/* Medical Identity, Blood Group & Insurance Details */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-1">
            {/* Blood Group */}
            <div className="p-4 rounded-2xl bg-rose-50/60 border border-rose-100 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-semibold uppercase tracking-wider text-rose-700 flex items-center gap-1.5">
                  <Droplet className="w-3.5 h-3.5 text-rose-500" />
                  Blood Group
                </span>
                <div className="text-lg font-black text-slate-900 mt-1">
                  {currentPatient.bloodGroup || 'Unknown'}
                </div>
              </div>
              <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${
                currentPatient.bloodGroup && currentPatient.bloodGroup !== 'Unknown'
                  ? 'bg-rose-100 text-rose-700'
                  : 'bg-amber-100 text-amber-700'
              }`}>
                {currentPatient.bloodGroup && currentPatient.bloodGroup !== 'Unknown' ? 'Confirmed Type' : 'Not Stated'}
              </span>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-teal-50/60 border border-teal-100 flex items-center justify-between">
              <div>
                <span className="text-[11px] font-semibold uppercase tracking-wider text-teal-700 flex items-center gap-1.5">
                  <CreditCard className="w-3.5 h-3.5 text-teal-600" />
                  National ID / Passport
                </span>
                <div className="text-sm font-mono font-bold text-slate-900 mt-1 truncate max-w-[160px]">
                  {currentPatient.nationalId || 'Not registered'}
                </div>
              </div>
              <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-teal-100 text-teal-700">
                Gov Record
              </span>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100 flex items-center justify-between">
              <div className="min-w-0">
                <span className="text-[11px] font-semibold uppercase tracking-wider text-indigo-700 flex items-center gap-1.5">
                  <Shield className="w-3.5 h-3.5 text-indigo-600" />
                  Health Insurance
                </span>
                <div className="text-xs font-bold text-slate-900 mt-1 truncate">
                  {currentPatient.hasInsurance
                    ? currentPatient.insuranceProvider || 'Insured'
                    : 'Out of Pocket / Self-Pay'}
                </div>
                {currentPatient.hasInsurance && currentPatient.insurancePolicyNumber && (
                  <p className="text-[10px] font-mono text-slate-500 mt-0.5 truncate">
                    Pol: {currentPatient.insurancePolicyNumber}
                  </p>
                )}
              </div>

              {currentPatient.insuranceCardUrl && (
                <button
                  onClick={() => setShowInsuranceCardModal(true)}
                  className="px-2.5 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-bold shadow-xs flex items-center gap-1 shrink-0 cursor-pointer"
      ...
      {showInsuranceCardModal && currentPatient?.insuranceCardUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm">
          <div className="bg-white rounded-3xl max-w-xl w-full p-6 shadow-2xl border border-slate-200 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-slate-100">
              <div className="flex items-center gap-2">
                <ShieldCheck className="w-5 h-5 text-indigo-600" />
                <h3 className="font-bold text-slate-900 text-base">Patient Insurance Card</h3>
              </div>
              <button
                onClick={() => setShowInsuranceCardModal(false)}
            ...
            <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 text-xs text-slate-600 space-y-1">
              <p><strong>Insured:</strong> {currentPatient.name}</p>
              <p><strong>Provider:</strong> {currentPatient.insuranceProvider || 'N/A'}</p>
              {currentPatient.insurancePolicyNumber && (
                <p><strong>Policy ID:</strong> <span className="font-mono font-semibold">{currentPatient.insurancePolicyNumber}</span></p>
              )}
            </div>

            <div className="rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 flex items-center justify-center min-h-[220px] max-h-[400px]">
              {currentPatient.insuranceCardUrl.startsWith('data:application/pdf') || currentPatient.insuranceCardUrl.endsWith('.pdf') ? (
                <div className="p-6 text-center space-y-3">
                  <FileText className="w-12 h-12 text-indigo-600 mx-auto" />
                  <p className="text-xs font-semibold text-slate-700">PDF Document Attached</p>
                  <a
                    href={currentPatient.insuranceCardUrl}
                    target="_blank"
                    rel="noreferrer"
                ...
                <img
                  src={currentPatient.insuranceCardUrl}
                  alt="Insurance Card"
