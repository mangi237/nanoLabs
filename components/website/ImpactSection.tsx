...
export const ImpactSection: React.FC = () => {
  ...
  return (
    <section id="impact" className="py-24 bg-[#0A2C27]/40 relative overflow-hidden border-t border-white/5">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {impacts.map((imp, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-6 rounded-3xl bg-[#041C18] border border-white/10 hover:border-white/20 transition-all duration-300 hover:shadow-2xl flex flex-col justify-between space-y-4"
              >
                <div className="space-y-3">
                  ...
                  <div className="text-xs font-bold text-[#2DD4BF] leading-tight">
                    {imp.subtitle}
                  </div>
        ...
        <div className="mt-12 grid grid-cols-2 lg:grid-cols-4 gap-4">
          {siteConfig.impactStats.map((stat, sIdx) => (
            <div
              key={sIdx}
              className="p-5 rounded-2xl bg-[#041C18]/80 border border-white/10 text-center space-y-1"
            >
              <div className="text-2xl sm:text-3xl font-black text-white font-mono">
                {stat.value}
              </div>
              <div className="text-xs font-bold text-[#2DD4BF]">{stat.label}</div>
              <p className="text-[11px] text-[#93B4AF]">{stat.description}</p>
            </div>
          ))}
        </div>
      </div>
