...
export const TeamSection: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-nl-off relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-3 max-w-2xl mx-auto mb-14">
          ...
          <h2 className="text-2xl sm:text-4xl font-black text-nl-ink tracking-tight font-display">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {team.map((m, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="p-6 rounded-3xl bg-white border border-slate-200 hover:border-nl-teal/40 transition-all duration-300 hover:shadow-xl space-y-3"
            >
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-nl-mint text-nl-deep">{m.badge}</span>
              <h3 className="text-base font-black text-nl-ink font-display">{m.name}</h3>
              <div className="text-xs font-bold text-nl-blue">{m.role}</div>
