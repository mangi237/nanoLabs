    ...
    <section className="py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center space-y-4 max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-3xl sm:text-5xl font-black text-nl-ink tracking-tight leading-tight font-display">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-16">
          {challenges.map((c, i) => (
            <motion.div
              ...
              className="p-4 rounded-2xl bg-nl-off border border-slate-200 hover:border-nl-danger/30 transition-all"
            >
              <c.Icon className="w-5 h-5 text-nl-danger/70 mb-2" />
              <div className="text-2xl font-black font-display text-nl-ink">
                <Counter value={c.stat} />
              </div>
              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mt-1">
                {c.title}
              </div>
              <p className="text-[10px] text-slate-500 leading-snug mt-2">{c.desc}</p>
            </motion.div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
          <motion.div
            ...
            className="lg:col-span-6 rounded-3xl bg-nl-off border border-slate-200 p-6 sm:p-8 space-y-5"
          >
            <div className="flex items-center gap-2.5">
              <span className="w-3 h-3 rounded-full bg-slate-400" />
              <h3 className="text-lg font-black text-nl-ink font-display">The old way</h3>
            </div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="lg:col-span-6 rounded-3xl bg-gradient-to-br from-nl-deep via-nl-teal to-nl-light p-6 sm:p-8 space-y-5 shadow-2xl relative overflow-hidden text-white"
          >
            <div className="absolute top-0 right-0 w-40 h-40 bg-nl-glow/30 rounded-full blur-3xl pointer-events-none" />
            ...
            <div className="space-y-3 relative">
              {solutions.map((s, i) => (
                ...
                >
                  ...
                  <p className="text-[11px] text-white/85 pl-6 leading-relaxed">{s.desc}</p>
                </motion.div>
              ))}
            </div>
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-3">
          {[
            ...
            <motion.div
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-nl-off border border-slate-200 text-xs text-slate-600"
            >
              <p.Icon className="w-3.5 h-3.5 text-nl-teal" />
              {p.label}
            </motion.div>
          ))}
        </div>
