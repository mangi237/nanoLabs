...
import {
  UserPlus, TestTubes, Truck, FileEdit, CheckCircle, Send, ArrowRight,
} from 'lucide-react';
...
const steps = [
  ...
  { num: '03', title: 'In transit', desc: 'For home collection, the phlebotomist\u2019s phone pings GPS. The patient sees the movement live.', Icon: Truck, color: '#2DD4BF' },
  ...
  { num: '06', title: 'Report delivered', desc: 'Lab-branded PDF with QR to the audit chain. Patient is notified. Doctor sees it on share.', Icon: Send, color: '#2DD4BF' },
];
...
export const WorkflowSection: React.FC<WorkflowSectionProps> = ({ onGoToPortal }) => {
  ...
  return (
    <section id="workflow" className="py-24 bg-nl-ink relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            return (
              <motion.div
                ...
                className={`group p-6 rounded-3xl border transition-all cursor-pointer flex flex-col justify-between space-y-4 ${
                  isActive
                    ? 'bg-white/10 border-nl-glow/60 shadow-xl shadow-nl-glow/10'
                    : 'bg-nl-black border-white/10 hover:border-white/25'
                }`}
              >
                <div className="flex items-center justify-between">
                  <span className={`text-2xl font-black font-mono transition-colors ${isActive ? 'text-nl-glow' : 'text-white/20'}`}>
                    {step.num}
                  </span>
                  <motion.div
                ...
                <div className="space-y-2">
                  <h3 className={`text-lg font-black transition-colors font-display ${isActive ? 'text-nl-glow' : 'text-white'}`}>
                    {step.title}
                  </h3>
                ...
                <div className="pt-2 border-t border-white/10 flex items-center justify-between text-[11px] font-mono text-white/40">
                  ...
                  <span className={isActive ? 'text-nl-glow font-bold' : ''}>
        ...
        <div className="mt-12 text-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-white/5 border border-white/15 hover:border-nl-glow/40 text-white font-bold text-xs transition-all hover:scale-[1.03] cursor-pointer"
          >
            <span>See the live workflow</span>
            <ArrowRight className="w-4 h-4 text-nl-glow" />
          </button>
        </div>
      </div>
    </section>
