...
export const LabReportPdfViewModal: React.FC<LabReportPdfViewModalProps> = ({
  ...
  const labName = labInfo?.name || booking.labName || booking.labDetails?.name || 'nanoLabs Clinical Diagnostics Center';
  const labSlogan = labInfo?.slogan || labInfo?.tagline || booking.labDetails?.slogan || 'Accurate • Precision • Verified Clinical Diagnostics';
  const labAddress = labInfo?.address || labInfo?.location || booking.labAddress || booking.labDetails?.address || 'Central Diagnostic Facility';
  const labPhone = labInfo?.phone || labInfo?.contactNumber || booking.labPhone || booking.labDetails?.phone || '+237 600 000 000';
