...
export const MedicalReceiptModal: React.FC<MedicalReceiptModalProps> = ({
  ...
  // Get ONLY validated/receptionist confirmed tests
  const validatedTests = (booking.tests || []).filter(t => t.receptionistValidated === true);
  const totalValidatedAmount = validatedTests.reduce((sum, t) => sum + (t.price || 0), 0);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md overflow-y-auto">
        <div className="bg-slate-900 border border-slate-700 text-slate-900 rounded-3xl max-w-4xl w-full p-4 sm:p-6 shadow-2xl relative animate-in zoom-in-95 duration-150 my-auto max-h-[96vh] flex flex-col">
          ...
          <div className="overflow-y-auto flex-1 p-1 sm:p-4 bg-slate-100 my-2 rounded-2xl print:p-0 print:m-0 print:bg-white print:overflow-visible">
            <div id="receipt-content" className="bg-white rounded-xl shadow-lg border border-slate-300 p-6 sm:p-8 max-w-3xl mx-auto font-sans text-slate-900 print:shadow-none print:border-none print:max-w-none print:p-8 print:border-2 print:rounded-none space-y-4">
              ...
              {/* Itemized Tests - ONLY validated tests */}
              <div>
                ...
                <table className="w-full text-left text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-200">
                    {validatedTests.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan={3} className="py-4 text-center text-slate-500">No validated tests to display</td>
                      </tr>
                    )}
                  </tbody>
                  <tfoot>
                    ...
                    {paymentDetails?.insuranceCoveragePercent && paymentDetails.insuranceCoveragePercent > 0 && (
                      <tr className="text-blue-700 font-bold">
                        ...
                        <td className="py-1 px-3 text-right font-mono">-{Math.round((totalValidatedAmount || 0) * (paymentDetails.insuranceCoveragePercent / 100)).toLocaleString()}</td>
                      </tr>
                    )}
                    <tr className="border-t-2 border-slate-400 text-base font-black">
                      ...
                      <td className="py-3 px-3 text-right font-mono text-emerald-700">
                        {(paymentDetails?.actualPaidAmount || totalValidatedAmount || 0).toLocaleString()} FCFA
                      </td>
                    </tr>
                  </tfoot>
                </table>
              ...
              {/* Footer */}
              <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between text-[10px] text-slate-500 border border-slate-200 print:bg-gray-50">
                <div className="flex items-center gap-2">
